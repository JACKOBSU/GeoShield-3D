...
def export_high_hazard_points(
    ...
    try:
        if ext == ".str":
            ...
            # Standard Surpac coordinate order: String_No, Y (North), X (East), Z (Elevation), Attribute
            # Header lines are standard for Surpac string files
            with open(file_path, "w") as f:
                ...
                for _, row in high_hazard_df.iterrows():
                    # We output string number 99 for high hazard, Y (North), X (East), Z, and Hazard Index
